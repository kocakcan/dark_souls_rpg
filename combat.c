...
int random_percent(void) { return rand() % 100; }
...
void cast_spell(Character *caster, Character *target) {
  ...
  if (strcmp(caster->spell, "Soul Arrow") == 0) {
    ...
    printf(
        "🔥 %s casts Fireball! %s takes %d fire damage and starts burning!\n",
    ...
    printf(
        "🤢 %s casts Poison Mist! %s takes %d poison damage and is poisoned!\n",
        caster->name, target->name, damage);
    target->hp -= damage;
    if (target->hp < 0)
      target->hp = 0;
